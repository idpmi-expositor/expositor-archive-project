...
canonical only after placeholders are replaced, automated values are
...
9. Confirm automated section metadata includes section traces and remains
   marked unreviewed.
10. Confirm generated draft YAML is stored under
   `archive/drafts/<publication_id>/`, not `archive/lessons`.
11. Before promotion, confirm each lesson YAML preserves `source_trace`,
